(function() {
    ...
    // Generate PDF - FIXED with proper positioning and timing
    function generatePDFBlob() {
        return new Promise((resolve, reject) => {
            ...
            // Ensure child elements inside are visible when capturing
            const innerWrapper = pdfContainer.firstElementChild;
            if (innerWrapper) {
                ...
                setTimeout(function() {
                    html2canvas(pdfContainer, {
                        ...
                        onclone: function(clonedDoc) {
                            // Ensure cloned elements have proper visibility
                            const clonedContainer = clonedDoc.querySelector('[style*="visibility: hidden"]');
                            if (clonedContainer) {
    ...
    function downloadPDF() {
        ...
        generatePDFBlob()
            ...
            .catch((error) => {
                ...
                showToast(error.message || 'Error generating PDF. Please try again.', 'error');
            });
    }
...
Generated using Invoice & Receipt Generator
                ...
                showToast('Email opened. PDF downloaded for attachment.', 'success');
                closeShareModal();
            })
            .catch((error) => {
    ...
    function shareViaGmail() {
        const summary = getDocumentSummary();
        showToast('Generating PDF for Gmail...', 'info');
        generatePDFBlob()
            .then(({ blob, filename }) => {
                const subject = `${summary.type} ${summary.number} for ${summary.client}`;
                const body = `
